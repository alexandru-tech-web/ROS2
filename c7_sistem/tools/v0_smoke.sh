...
set -u
OUT="${1:?out_dir}"; DUR="${2:-60}"; MODDT="${3:-max}"; mkdir -p "$OUT/c6" "$OUT/c3_jurnal"
...
setsid ros2 launch c7_sistem v0_sistem.launch.py jurnal:="$OUT/c3_jurnal" c6_outputs:="$OUT/c6" durata_app:="$DUR" mod_dt:="$MODDT" > "$OUT/launch.log" 2>&1 &
LP=$!
sleep 12
ros2 node list --no-daemon > "$OUT/node_list.txt" 2>&1
ros2 topic list --no-daemon > "$OUT/topic_list.txt" 2>&1
for t in /network_confidence /c6/cmd_op /c6/pose /teleop/cmd /teleop/pose /sar/telemetry /mesh/relay /mesh/beacon /sar/telemetry_mesh /sar/telemetry/d1 /c3/app /c3/tx; do
  timeout 12 ros2 topic hz -w 20 "$t" > "$OUT/hz_$(echo $t | tr '/' '_').txt" 2>&1 || true
done
timeout 15 ros2 topic echo --once /mesh/relay > "$OUT/relay_once.txt" 2>&1 || true      # un pachet releat, cu path (salturi)
timeout 15 ros2 topic echo --once /sar/telemetry_mesh > "$OUT/egress_once.txt" 2>&1 || true
...
echo "relay_salturi=$(/usr/bin/grep -F "[punte_telemetrie" "$OUT/launch.log" | tail -1 | /usr/bin/grep -o "in=[0-9]* out=[0-9]*")" >> "$OUT/rezumat.txt"
